...
```{r CO2 map, message=FALSE}
source("./LargeTank_CO2_Trend/CO2_Map.R")
pH_Trend_Both_Chambers
```

\
**Fig 1A. The pH trend in both chambers.** The two chambers experienced relatively little water exchange across the connecting channel due to the circular chamber design and opposing currents.
...
\
**Fig 1B. Model to predict pCO~2~ from pH.** The relationship between pH and dissolved carbon dioxide in the large tank.
...
Fig1 <- ggarrange(pH_Trend_Both_Chambers, Model_to_predict_pCO2_from_pH,
                  labels = c("A","B"),
                  ncol = 1, nrow = 2,
                 # align = "v",
                  widths = c(1.5,1))
...
**Table 2. Dissolved carbon dioxide avoidance and ambient water parameters.** The pCO~2~ at last exit is estimated from instantaneous pH in both tanks.
...
tiff("./Figures/TIF/Fig5_CO2_Avoidance_Boxplot_1.tif", width = 1800, height = 1200, units = "px", res = 300, compression = c("none"))
CO2_Avoidance_Boxplot_1
dev.off()

ggsave(file = "./Figures/EPS/Fig5_CO2_Avoidance_Boxplot_1.eps",
...
This code was used to predict the pH to reach during conditioning treatments.
...
There was not a significant difference in dissolved CO~2~ avoidance thresholds between species (`r modSpecies$method`, Χ^2^ = `r chi2Species`, p = `r pvalSpecies`) or tank size (`r modTank$method`, Χ^2^ = `r chi2Tank`, p = `r pvalTank`, Fig 5).
Across all trials in both tanks, bigheaded carp avoided pCO~2~ at approximately `r avoidPPM` ppm.
This threshold pCO~2~ level was used for CO~2~ conditioning treatments by reaching an instantaneous pH of approximately `r pHThreshold` by the conditioning treatment midpoint in the small tank and endpoint in the large tank.
...
\
**Fig 5. CO~2~ avoidance boxplots.** 
Carp avoided pCO~2~ at `r avoidPPM` ppm on average.
Boxplots include median ± quartiles for each tank size.  
Whiskers extend to 1.5 times IQR. 
Dark points are bighead, and light points are silver carp.
Circles are individuals, and triangles are schools of five fish. 
Bigheaded carps exited the increasing pCO~2~ chamber for the last time at the same threshold level in the small and large tanks (`r modTank$method`, Χ^2^ = `r chi2Tank`, p = `r pvalTank`).
\
...
In the large shuttle tank, individual bighead carp (n = 10) exhibited CO~2~ avoidance at `r largeBighead` ± `r largeBighead_SE` ppm (Table 2).
...
**Table 2. Carbon dioxide avoidance and ambient water parameters in both tanks.** The pCO~2~ is estimated from instantaneous pH at the last exit and measured from water samples.
...
XXX Unused code below, cut before final archiving XXX
