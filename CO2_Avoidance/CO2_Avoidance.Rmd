...
\
**Fig 2. Model to predict pCO~2~ from pH.** The relationship between pH and dissolved carbon dioxide in the large tank.
...
chi2Tank <- format(round(as.numeric(modTank$statistic), 2), nsmall = 2, big.mark = ",")
pvalTank <- format(round(as.numeric(modTank$p.value), 2), nsmall = 2)

chi2Species <- format(round(as.numeric(modSpecies$statistic), 2), nsmall = 2, big.mark = ",")
pvalSpecies <- format(round(as.numeric(modSpecies$p.value), 2), nsmall = 2)

avoidPPM <- format(signif(as.numeric(avoidAvgBoth), 3), big.mark = ",")
...
\
**Fig 5. CO~2~ avoidance boxplots.** Bigheaded carps exited the increasing pCO~2~ chamber for the last time at the same threshold level in the small and large tanks (`r modTank$method`, Χ^2^ = `r chi2Tank`, p = `r pvalTank`).
