...
# library('agricolae') #value_mean hsd
...
mod <- lmer(Exit_s ~ TrainedWith*Sound*Day + (1|FishID), data = dataNoOut)
Anova(mod)
```
...
mod1 <- lmer(Return_s ~ TrainedWith*Sound + Day + (1|FishID), data = dataNoOut)
Anova(mod1)
#summary(mod1)

mod2 <- lmer(Return_s ~ TrainedWith*Sound*Day + (1|FishID), data = dataNoOut)
Anova(mod2)
...
dataNoOut <- dataNoOut %>%  filter(ID != '45') # because trial 44 was removed earlier, 45 is also as the pair
