...
Additionally, paired t-tests are run on behavioral metrics from the fish tracking software.
...
largeBoxplotExit <- ggboxplot(
  ...
  labs(x = "Trained With", y = "Time to Exit Sound Chamber (s)") +
  theme_classic2() +
...
largeBoxplotReturn <- ggboxplot(
  ...
  labs(x = "Conditioned With", y = "Time (s) in Opposite Chamber until Return") +
  theme_classic2() +
  scale_fill_brewer(palette = "Greys", labels = c('Before','During')) +
  stat_pvalue_manual(pwc, label = "p.adj.signif", tip.length = 0, hide.ns = FALSE) +
  labs(
...
```
\
**Fig Not moving. Stationary time (%).** Carp from both training treatments often exhibited freezing responses to sound playback. 
