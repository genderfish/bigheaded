...
Sound had a significant effect on time until first exit (Mixed ANOVA, Χ^2^ = `r soundChisq`, p < `r soundPval`).
...
smallboxplotReturn <- ggboxplot(data3, 
          x = "Day", #org data 3
          y = "Return_s",
          fill = "Trained",
          outlier.shape = NA,
...
Conditioning with CO~2~ had a marginally significant effect on seconds to return to the sound chamber (Mixed ANOVA, Χ^2^ = `r trainChisq`, p = `r trainPval`).
Presence of the acoustic signal had a highly significant effect on time to return (Χ^2^ = `r soundChisq`, p = `r soundPval`).
Additionally, the number of days since conditioning had a significant effect on time to return (Χ^2^ = `r dayChisq`, p = `r dayPval`).
Notably, there was a significant three-way interaction between conditioning treatment, presence of sound, and the number of days since trial onset (Χ^2^ = `r interactChisq`, p = `r interactPval`).
