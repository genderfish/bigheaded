...
data <- read.csv("SmallTank_Exp2_2022_10_25.csv") %>% 
  ...
  select(-c(Stopped_Sound_On_Return, Avoids_N, Returns_N, Quiet_Chamber_.,First_Return_s)) %>% 
  rename(
...
data$Side <- gsub("R", "Right", data$Side)
data$Side <- gsub("L", "Left", data$Side)
...
str(data) # Is everything the right variable type?

# Convert other variable types
data$First_Exit_s <- as.integer(data$First_Exit_s)
data$First_Quiet_Side_s <- as.integer(data$First_Quiet_Side_s)
data$Angle <- as.integer(data$Angle)
data$Temp <- as.numeric(data$Temp)

data$logQuiet <- log(data$First_Quiet_Side_s)
data$logExit <- log(data$First_Exit_s)
...
data2 <- data %>% filter(!is.na(First_Exit_s), !is.infinite(First_Exit_s))

# identify outliers 
outliers <- data2 %>% 
  group_by(Day, Trained, Sound) %>%
  identify_outliers(First_Exit_s) # beyond 1.5 x IQR
...
ggqqplot(dataNoOut, "First_Exit_s", ggtheme = theme_bw()) +
...
dataNoOut %>%
  group_by(Day) %>%
  levene_test(First_Exit_s ~ Trained)
# if p > .05 for each day assumption is met

# Homogeneity of covariances assumption
box_m(dataNoOut[, "First_Exit_s", drop = FALSE], dataNoOut$Trained)
...
# no outliers
mod1 <- lmer(First_Exit_s ~ Trained*Sound + Days_Since_Train + (1|School_ID), data = dataNoOut)
Anova(mod1)

mod2 <- lmer(First_Exit_s ~ Trained*Sound*Days_Since_Train + (1|School_ID), data = dataNoOut)
Anova(mod2)
...
# no extreme outliers
mod3 <- lmer(First_Exit_s ~ Trained*Sound + Days_Since_Train + (1|School_ID), data = dataNoExtremeOut)
Anova(mod3)

mod4 <- lmer(First_Exit_s ~ Trained*Sound*Days_Since_Train + (1|School_ID), data = dataNoExtremeOut)
Anova(mod4)
...
SmallboxplotExit <- ggplot(data3, aes(x = Day,y = First_Exit_s, fill = Trained)) +
...
modDay1 <- t.test(data = data4, First_Exit_s ~ Trained)
```
...
data2 <- data %>% filter(!is.na(First_Quiet_Side_s), !is.infinite(First_Quiet_Side_s))

# identify outliers (both species)
outliers <- data2 %>% 
  group_by(Days_Since_Train, Trained, Sound) %>%
  identify_outliers(First_Quiet_Side_s)

extremeOutliers <- outliers %>% filter(is.extreme == TRUE)

dataNoOut <- data2 %>% anti_join(outliers) # remove outliers
dataNoExtremeOut <- data2 %>% anti_join(extremeOutliers)

# normality assumption
ggqqplot(dataNoOut, "First_Quiet_Side_s", ggtheme = theme_bw()) +
  facet_grid(Day ~ Trained) 
...
dataNoOut %>%
  group_by(Day) %>%
  levene_test(First_Quiet_Side_s ~ Trained)
# if p > .05 for each day assumption is met

# Homogeneity of covariances assumption
box_m(dataNoOut[, "First_Quiet_Side_s", drop = FALSE], dataNoOut$Trained)
# if p is > 0.001 this assumption is met
# Note: Box's M test is very sensitive
```
...
# mixed ANOVA computation
mod1 <- lmer(First_Quiet_Side_s ~ Trained*Sound + Days_Since_Train + (1|School_ID), data = dataNoOut)

mod2 <- lmer(First_Quiet_Side_s ~ Trained*Sound*Days_Since_Train + (1|School_ID), data = dataNoOut)
...
smallboxplotReturn <- ggplot(data3, aes(x = Day,y=First_Quiet_Side_s, fill = Trained)) +
  geom_boxplot(position = position_dodge(1)) +
  stat_compare_means(aes(group = Trained), method = "t.test", 
                     label = "p.signif", hide.ns = TRUE) + 
...
# T test for day 1
data4 <- data3 %>% filter(Day == 1)
modDay1 <- t.test(data = data4, First_Quiet_Side_s ~ Trained)
modDay1

# T test for day 7
data4 <- data3 %>% filter(Day == 7)
modDay7 <- t.test(data = data4, First_Quiet_Side_s ~ Trained)
modDay7
```
...
modTrained <- t.test(data = data4, First_Quiet_Side_s ~ Day)
modTrained

data4 <- data3 %>% filter(Trained == '0 days') %>% filter(Day == 1 | Day == 4)
moduntrained <- t.test(data = data4, First_Quiet_Side_s ~ Day)
moduntrained
```
...
p <- ggplot(dataNoOut, aes(x=Days_Since_Train,y=First_Quiet_Side_s, color=Trained)) + geom_point() +
...
group_by(Sound,Trained,Day) %>%
  summarize(
    Quiet_s = mean(First_Quiet_Side_s),
    Quiet_s_sd = sd(First_Quiet_Side_s))
